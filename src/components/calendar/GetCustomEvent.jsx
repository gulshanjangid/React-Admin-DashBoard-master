...
export default function GetCustomEvent({ event }) {
  ...
  const start = `${formatTime(event?.start.getHours())}:${
    event?.start.getMinutes() > 9
      ? event?.start.getMinutes()
      : "0" + event?.start.getMinutes()
  }`;
  const end = `${formatTime(event?.end.getHours())}:${
    event?.end.getMinutes() > 9
  ...
  return view === "month" ? (
    ...
    <div
      className={`${getWeekClass(
        label
      )} border-l-4 bg-opacity-40 rounded-tr-2xl rounded-br-2xl w-full h-full flex items-center justify-center pl-2`}
    >
      <div style={{ lineHeight: "1.2rem" }} className="text-lg font-semibold">
        {title}
      </div>
    ...
    <div
      className={`${getWeekClass(
        label
      )} border-l-4 pl-6 bg-opacity-40 rounded-tr-2xl rounded-br-2xl w-full h-full flex flex-col items-start justify-evenly`}
    >
      <div className="text-2xl font-semibold">{title}</div>
      <div className="time text-lg">
        {start} - {end}
      </div>
    </div>
  );
